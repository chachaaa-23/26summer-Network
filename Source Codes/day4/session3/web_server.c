...
int *request_handler(void *arg) {
  ...
  clnt_read = fdopen(clnt_sock, "r"); // fd 로 file open
  clnt_write =
      fdopen(dup(clnt_sock), "w"); // 이미 read 에서 썻으니까 dup 통해 복제
...
void send_error(FILE *fp) {
  ...
  char content[] = "<html><head><title>NETWORK</title></head>"
                   "<body><font size=+5><br>Error! Please re-check the file "
                   "name and requet method"
                   "</font></body></html>";
